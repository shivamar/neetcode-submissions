class Solution {
  ...
  public int[]  findOrder(int numCourses, int[][] prerequisites) {
    ...
    ArrayList<Integer> resultLi = new ArrayList<Integer>();
    ...
    //  Or depMap.computeIfAbsent(pair[1], k -> new ArrayList<>()).add(pair[0]);
    //   ArrayList<Integer> arrList = depMap.getOrDefault(pair[1], new
    //   ArrayList<Integer>()); 
    //    arrList.add(pair[0]); 
    ...
    while (!queue.isEmpty()) {
      ...
      // System.out.println( removedDep + " " + dependencies.toString()); //debug only- comment before submit

      for (int dep : dependencies) {
        int newCount =inVectorCounts.get(dep) - 1;
        inVectorCounts.put(dep,newCount);
   ...
   if(finishedCounts != numCourses) return new int[0];

    return resultLi.stream()
                  .mapToInt(Integer::intValue)
                  .toArray();

    // if (finishedCounts == numCourses)
    //   return true;
