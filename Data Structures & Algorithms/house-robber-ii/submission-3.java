class Solution {
    ...
    public int dfs(int[] nums, int index, boolean flag){
        if(index > nums.length-1 || (flag && index==nums.length-1)) return 0;

        if(dp[index] != -1) return dp[index];

        dp[index] = Math.max(dfs(nums,index+1,flag), nums[index] + dfs(nums,index+2,(flag || index==0) ));
        return dp[index];
