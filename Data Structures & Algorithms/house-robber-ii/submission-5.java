class Solution {
    ...
    public int dfs(int[] nums, int index, int flag){
        if(index > nums.length-1 || (flag==1 && index==nums.length-1)) return 0;

        if(dp[index][flag] != -1) return dp[index][flag];

        dp[index][flag] = Math.max(dfs(nums,index+1,flag), nums[index] + dfs(nums,index+2,(flag==1 || index==0) ? 1 : 0 ));
